...
fireWrap.innerHTML = 'Fire <div class="bar-container"><div id="fireBar" class="bar-fill fire-fill" style="width: 100%;"></div></div>'; barsGroup.appendChild(fireWrap);
const energyWrap = document.createElement("div"); energyWrap.className = "bar-wrapper";
energyWrap.innerHTML = 'Energy <div class="bar-container"><div id="energyBar" class="bar-fill energy-fill" style="width: 100%;"></div></div>'; barsGroup.appendChild(energyWrap);
const healthWrap = document.createElement("div"); healthWrap.className = "bar-wrapper";
healthWrap.innerHTML = 'Health <div class="bar-container"><div id="healthBar" class="bar-fill health-fill" style="width: 100%;"></div></div>'; barsGroup.appendChild(healthWrap);
bottomRow.appendChild(barsGroup);

const scoreWrap = document.createElement("div"); scoreWrap.className = "score-text";
scoreWrap.innerHTML = `Day <span id="dayText">1</span> | Score: <span id="scoreText">0</span> |
<span style="color:#ffd700;">🟡 <span id="goldenWoodText">0</span></span>`;
...
walkIdleSprite.onload = () => { const inferred = inferSpriteFrameSize({ img: walkIdleSprite, preferred: 32, min: 16, max: 64, rowsHint: 4 });
if (inferred) { spriteFrames.walk.w = inferred.w; spriteFrames.walk.h = inferred.h; } };
dieSprite.onload = () => { const inferred = inferSpriteFrameSize({ img: dieSprite, preferred: 32, min: 16, max: 96, rowsHint: 1 });
if (inferred) { spriteFrames.die.w = inferred.w; spriteFrames.die.h = inferred.h; } configureDeathClip(); };
const fireAnim = { frameWidth: 64, frameHeight: 64, cols: 10, rows: 6, frameCount: 60, frameTimer: 0 };
fireSprite.onload = () => { const fullW = fireSprite.width || 0; const fullH = fireSprite.height || 0;
if (fullW > 0 && fullH > 0) { fireAnim.frameWidth = fullW / fireAnim.cols; fireAnim.frameHeight = fullH / fireAnim.rows;
fireAnim.frameCount = fireAnim.cols * fireAnim.rows; } };
...
function inferSpriteFrameSize({ img, preferred, min, max, rowsHint }) { const w = img.naturalWidth || 0; const h = img.naturalHeight ||
0; if (!w || !h) return null; const candidates = [];
...
let ty = Math.random() * h; if (dist({ x: tx, y: ty }, state.fire) > 230) { state.trees.push({ x: tx, y: ty, r: 15 + Math.random() * 25, color: Math.random() > 0.5 ? "#142e12" : "#1a3a17" });
} } }
function resizeCanvas() { const ratio = window.devicePixelRatio || 1; const rect = canvas.getBoundingClientRect(); canvas.width = Math.floor(rect.width * ratio);
...
state.fire.y = rect.height * 0.5 + 40; state.tent.x = state.fire.x; state.tent.y = state.fire.y - 85; if (state.trees.length === 0) generateTrees();
}
...
function randomWood() { 
  ...
  for (let i = 0; i < 20; i += 1) { const wood = { x: 20 + Math.random() * (w - 40), y: 20 + Math.random() * (h - 40), r: 10, angle: Math.random() * Math.PI * 2, isGolden: isGold };
if (dist(wood, state.fire) > state.fire.r + 60 && dist(wood, state.player) > state.player.r + 30) return wood;
} 
...
if (dist(mushroom, state.fire) > state.fire.r + 80 && dist(mushroom, state.player) > state.player.r + 30) return mushroom;
...
state.mushroomRespawnTimer = 0; for (let i = 0; i < state.targetMushroomCount; i += 1) state.mushrooms.push(randomMushroom());
}
...
if (fireBar) fireBar.style.width = Math.max(0, state.fire.level) + "%";
...
state.pendingMushroomRespawns += mCollected;
  if (state.blueMushroom && dist(state.player, state.blueMushroom) <= state.player.r + state.blueMushroom.r) { state.superModeTimer = 8.0; playSound(sounds.feed); state.score += 100;
state.floatingTexts.push({ x: state.player.x, y: state.player.y - 30, text: "SUPER!", life: 2.0, color: "#00ffff" }); state.blueMushroom = null;
...
} } else { state.player.speed = 170; }
  for (let i = state.playerTrails.length - 1; i >= 0; i--) { state.playerTrails[i].life -= dt;
if (state.playerTrails[i].life <= 0) state.playerTrails.splice(i, 1); }
  
  if (isMoving) { if (Math.abs(moveX) > Math.abs(moveY)) state.player.dir = moveX > 0 ?
...
if (dTent < state.player.r + state.tent.r - 10) { let angle = Math.atan2(state.player.y - state.tent.y, state.player.x - state.tent.x);
...
0.6 * darkFactor : 0.15; const firePower = state.fire.level / 100; const flicker = Math.sin(state.fire.animationTimer * 0.5) * 8;
